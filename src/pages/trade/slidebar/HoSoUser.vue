<template>
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar style="height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 10px)" class="scroll-area--data-list-add-new">
        ...
        <div class="vx-vx-row mb-3">
          <div class="vx-col w-full">
            <div class="flex flex-wrap items-center mb-base avatar">
              <vs-avatar v-if="!avatar" :src="require('@/assets/images/avatar/da.jpg')" size="80px" class="mr-4" />
              ...
              <div>
                ...
                <vs-button type="border" color="#F1DD48" class="mr-4" @click="$refs.update_avatar_input.click()"> {{$t('trade.8')}} </vs-button>
              </div>
            </div>
          </div>
        </div>

        <h3 class="mb-3" style="font-weight: blod"><feather-icon icon="UserIcon"></feather-icon>
        {{$t('trade.9')}}
        </h3>
      ...
      <div class="p-4">
        <h3 class="mb-3" style="font-weight: blod"><feather-icon icon="UserCheckIcon"></feather-icon> Xác minh tài khoản</h3>
        <span class="d-flex mt-5" style="font-size: 12px"> Để đảm bảo an toàn cho tài sản của bạn, chúng tôi cần xác minh danh tính. Hãy điền thông tin thật chính xác, khi đã hoàn tất xác minh danh tính thì thông tin sẽ không chỉnh sửa được nữa. </span>
        <p class="clearfix"></p>
        <vs-button v-if="getDataJson.verify == 0" color="#F1DD48" type="border" class="mb-2 mt-2 float-right" @click.stop="showHoSoSetting()">Xác nhận</vs-button>
        <span v-else-if="getDataJson.verify == 2" class="lightyellow italic">* Tài khoản đang được xem xét</span>
      </div>

      <p class="clearfix"></p>
      <div class="p-4">
        <h3 class="mb-3" style="font-weight: blod"><feather-icon icon="LockIcon"></feather-icon> Bảo mật</h3>
        <span class="d-flex" style="font-size: 12px; margin-bottom: 5px"> Bạn có muốn thay đổi mật khẩu không? Nhấp vào nút phía dưới để thay đổi. </span>
        <p class="clearfix"></p>
        <vs-button color="#F1DD48" type="border" class="mb-2 float-right" @click="popupActiveChangePass = true">Đổi</vs-button>
        <p class="clearfix"></p>
      </div>

      <vs-popup background-color="rgba(0,0,0,.6)" :background-color-popup="colorxChangePass" title="Đổi mật khẩu" :active.sync="popupActiveChangePass">
        <div class="vx-vx-row">
          <div class="vx-col w-full">
          ...
          <div class="vx-col w-full mt-5">
            <small class="red italic" v-if="!getDataJson.c2fa && !DISABLE_2FA">* Bạn phải bật 2FA để điều chỉnh</small>
